...
def update(frameNum, img, grid, N):
    ...
    tBlock = block(newGrid, N)
    tBeehi = beehive(newGrid, N)
    tLoaf = loaf(newGrid, N)
    tBoat = boat(newGrid, N)
    tTub = tub(newGrid, N)

    tBlink = blinker(newGrid, N)
    tToad = toad(newGrid, N)
    tBeacon = beacon(newGrid, N)

    tGlider = glider(newGrid, N)
    tLight = lightWeight(newGrid, N)
    ...
    sumConfig = tBlock+tBeehi+tLoaf+tBoat+tTub+tBlink+tToad+tBeacon+tGlider+tLight #+tOthers

    perBlock = (tBlock*100)/sumConfig
    perBeehi = (tBeehi*100)/sumConfig
    perLoaf = (tLoaf*100)/sumConfig
    perBoat = (tBoat*100)/sumConfig
    perTub = (tTub*100)/sumConfig

    perBlink = (tBlink*100)/sumConfig
    perToad = (tToad*100)/sumConfig
    perBeacon = (tBeacon*100)/sumConfig

    perGlider = (tGlider*100)/sumConfig
    perLight = (tLight*100)/sumConfig

    #perOthers = (tOthers*100)/sumConfig

    global datas
    datas = datas + "\n###### Generation: " + str(gen) + " ######\n" + "Total Blocks: " + str(
        tBlock) + " Percentage: "+str(perBlock)+ "\nTotal Beehive: " + str(tBeehi) + " Percentage: "+str(perBeehi) + "\nTotal Loafs: " + str(tLoaf) + " Percentage: "+str(perLoaf)+ \
            "\nTotal Boats: " + str(tBoat)+ " Percentage: "+str(perBoat) + "\nTotal Tubs: " + str(tTub)+ " Percentage: "+str(perTub) + "\nTotal Blinkers: " + str(
        tBlink)+ " Percentage: "+str(perBlink) + "\nTotal Toads: " + str(tToad) + " Percentage: "+str(perToad)+ "\nTotal Beacons: " + str(tBeacon) + " Percentage: "+str(perBeacon)+ \
            "\nTotal Gliders: " + str(tGlider)+ " Percentage: "+str(perGlider) + "\nTotal Light-weight spaceship: " + str(tLight)+ " Percentage: "+str(perLight) +"\n" #+ "\nTotal others: " + str(tOthers) + " Percentage: "+str(perOthers)+ "\n"
    #print(datas)
